...
#if defined(NDEBUG)
# error "Oduwacoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "OduwaCoin Signed Message:\n";
...
CAmount GetProofOfWorkSubsidy(int nHeight, const CAmount& nFees)
{
...
CAmount GetProofOfStakeSubsidy(int64_t nCoinAge, int64_t nFees, int64_t bHeight)
{
	int64_t nSubsidy;


	if (bHeight <= 245450) {
		nSubsidy = nCoinAge * CENT * 33 / (365 * 33 + 8);
	}
	else {
	...
	CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
	...
	if (block.IsProofOfWork()) {
		hashProofOfStake = pindex->GetBlockHash();
	}
	if (block.IsProofOfStake()/* && chainparams.GetConsensus().IsProtocolV3(block.GetBlockTime())*/) {
		...
		if (!CheckStakeKernelHash(pindex->pprev, block.nBits, coins, prevout, block.vtx[1].nTime, hashProofOfStake, false))
			return state.DoS(100, error("ConnectBlock(): proof-of-stake hash doesn't match nBits"),
				REJECT_INVALID, "bad-cs-proofhash");
	}
	pindex->hashProof = hashProofOfStake;

	//if(pindex->pprev->GetBlockHash() == chainparams.GetConsensus().hashGenesisBlock)
	...
	//uint64_t nStakeModifier = ComputeStakeModifier(pindex->pprev, block.IsProofOfStake() ? block.vtx[1].vin[0].prevout.hash : block.GetHash());
	//pindex->nStakeModifier = nStakeModifier;
	...
	//// OduwaCoin also requires DER encoding of pubkeys
	//flags |= SCRIPT_VERIFY_DERKEY;
	//// OduwaCoin also requires low S in sigs
	...
	*/
	...
	if (block.IsProofOfWork()) {
		CAmount blockReward = nFees + GetProofOfWorkSubsidy(pindex->nHeight, nFees);
	...
	if (block.IsProofOfStake() /*&& chainparams.GetConsensus().IsProtocolV3(block.GetBlockTime())*/) {
		...
		CAmount blockReward = GetProofOfStakeSubsidy(nCoinAge, nFees, bHeight);
		...
		if (wallet.CreateCoinStake(wallet, block.nBits, 1, nFees, txCoinStake, key))
		{
			if (txCoinStake.nTime >= pindexBestHeader->GetPastTimeLimit() + 1)
			{
				...
				for (vector<CTransaction>::iterator it = block.vtx.begin(); it != block.vtx.end();)
					if (it->nTime > block.nTime) { it = block.vtx.erase(it); }
					else { ++it; }
